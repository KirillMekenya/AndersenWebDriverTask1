...
public class MissedHobbyRequiredFieldTest {
    public static void main(String[] args) throws ValueDoesNotMatch {
        ...
        try {
            ...
            submitButton.click();
            if (!driver.findElement(registrationFormElements.getMissedHobbyFieldLocator()).getText().equals("This field is required.")) {
                throw new ValueDoesNotMatch("Отсутствует валидация на заполнения обязательного поля Хобби");
            }
        } finally {
            driver.close();
        }
